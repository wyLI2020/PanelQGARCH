...
# library(parallel)
# source("EstimationProcedure_fr.R")
...
fr_rolling_forecast_summary_average <- function(forecast_summary, sig_level) {
  forecast_summary_average <- matrix(NA, nrow = 2, ncol = 5)
  rownames(forecast_summary_average) <- c("initial", "final")
  colnames(forecast_summary_average) <- c("ECR", "PE", "CC", "DQ", "CC+DQ")
  forecast_summary_average[1,1:2] <- apply(forecast_summary[1:2,], MARGIN = 1, FUN = mean)
  forecast_summary_average[1,3] <- mean(forecast_summary[3,] > sig_level)
  forecast_summary_average[1,4] <- mean(forecast_summary[4,] > sig_level)
  forecast_summary_average[1,5] <- mean((forecast_summary[3,] > sig_level) * (forecast_summary[4,] > sig_level))
  forecast_summary_average[2,1:2] <- apply(forecast_summary[5:6,], MARGIN = 1, FUN = mean)
  forecast_summary_average[2,3] <- mean(forecast_summary[7,] > sig_level)
  forecast_summary_average[2,4] <- mean(forecast_summary[8,] > sig_level)
  forecast_summary_average[2,5] <- mean((forecast_summary[7,] > sig_level) * (forecast_summary[8,] > sig_level))
  return(forecast_summary_average)
}

fr2_rolling_forecast_summary_average <- function(forecast_summary, sig_level) {
  forecast_summary_average <- matrix(NA, nrow = 2, ncol = 5)
  rownames(forecast_summary_average) <- c("initial", "final")
  colnames(forecast_summary_average) <- c("ECR", "PE", "CC", "DQ", "CC+DQ")
  forecast_summary_average[1,1:2] <- apply(forecast_summary[1:2,], MARGIN = 1, FUN = median)
  forecast_summary_average[1,3] <- mean(forecast_summary[3,] > sig_level)
  forecast_summary_average[1,4] <- mean(forecast_summary[4,] > sig_level)
  forecast_summary_average[1,5] <- mean((forecast_summary[3,] > sig_level) * (forecast_summary[4,] > sig_level))
  forecast_summary_average[2,1:2] <- apply(forecast_summary[5:6,], MARGIN = 1, FUN = median)
  forecast_summary_average[2,3] <- mean(forecast_summary[7,] > sig_level)
  forecast_summary_average[2,4] <- mean(forecast_summary[8,] > sig_level)
  forecast_summary_average[2,5] <- mean((forecast_summary[7,] > sig_level) * (forecast_summary[8,] > sig_level))
  return(forecast_summary_average)
