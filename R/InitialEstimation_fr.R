...
fit1_optim <-function(par=NULL,Y,w,tau,lower=c(NA,NA,1e-3),upper=c(NA,NA,1-1e-3),method="optim",iter_max_1=10,iter_max_2=20){
  # if(!is.na(seed)){
  #   set.seed(seed)
  # }
...
fr_phitilde_i_tau <- function(tau, y_i_T, w_i_T) {
  fit0 <- fit1_optim(Y=y_i_T, w=w_i_T, tau=tau)
  phitilde_i <- fit0$par
  return(phitilde_i)
}
