...
arma::vec fc_phitilde_i_tau(double tau, arma::vec y_i_T, arma::vec w_i_T) {
  ...
  NumericVector phitilde_i_nv = f(tau, y_i_T, w_i_T);
...
arma::mat fc_phitilde_N_tau(double tau, arma::mat Y_N_T, arma::mat w_N_T) {
  // the N \times 3 matrix of initial estimates with the i-th row being \widetilde{\bm{\phi}}_{i}(\tau)
  int N = Y_N_T.n_rows;
  arma::mat phitilde_3_N(3, N); phitilde_3_N.fill(0.0);
  arma::mat t_Y_T_N = Y_N_T.t();
  arma::mat t_w_T_N = w_N_T.t();
  for(int i = 0; i < N; i++) {
    arma::vec y_i_T = t_Y_T_N.col(i);
    arma::vec w_i_T = t_w_T_N.col(i);
    arma::vec phitilde_i = fc_phitilde_i_tau(tau, y_i_T, w_i_T);
    phitilde_3_N.col(i) = phitilde_i;
  }
  arma::mat phitilde_N_3 = phitilde_3_N.t(); 
  return phitilde_N_3;
}
